...
BEGIN
    ...
    SELECT
        ...
        to_date(s.data_situacao_cadastral,'YYYYMMDD'),
        s.motivo_situacao_cadastral,
        s.nome_cidade_exterior,
        s.pais,
        to_date(s.data_inicio_atividade,'YYYYMMDD'),
        ...
        s.email,
        s.situacao_especial,
        to_date(s.data_situacao_especial,'YYYYMMDD'),
        md5(
    ...
    SET
        ...
        data_situacao_cadastral = to_date(s.data_situacao_cadastral,'YYYYMMDD'),
        motivo_situacao_cadastral = s.motivo_situacao_cadastral,
        nome_cidade_exterior = s.nome_cidade_exterior,
        pais = s.pais,
        data_inicio_atividade = to_date(s.data_inicio_atividade,'YYYYMMDD'),
        ...
        data_situacao_especial = to_date(s.data_situacao_especial,'YYYYMMDD'),
        hash =
            md5(
                coalesce(s.nome_fantasia,'') ||
                coalesce(s.situacao_cadastral,'') ||
...
BEGIN
    ...
    SELECT
        ...
        to_date(s.data_entrada_sociedade, 'YYYYMMDD'),
        s.pais,
        ...
        md5(
            ...
            coalesce(s.data_entrada_sociedade,'') ||
            coalesce(s.pais,'') ||
            coalesce(s.representante_legal,'') ||
            coalesce(s.nome_representante,'') ||
            coalesce(s.qualificacao_representante,'') ||
    ...
    SET
        ...
        data_entrada_sociedade = to_date(s.data_entrada_sociedade,'YYYYMMDD'),
        pais = s.pais,
...
BEGIN
    ...
    SET
        ...
        opcao_mei               = s.opcao_pelo_mei,
        data_opcao_mei          = to_date(s.data_opcao_mei,'YYYYMMDD'),
        data_exclusao_mei       = to_date(s.data_exclusao_do_mei,'YYYYMMDD'),
...
BEGIN
    ...
    SELECT
        trim(s.codigo_int)::int,
        s.nome,
        md5(coalesce(s.nome,''))
    FROM cnpj.stage_municipios s
    LEFT JOIN cnpj.municipio f ON f.codigo = trim(s.codigo_int)::int
    ...
    SET
        ...
        atualizado_em = now()
    FROM cnpj.stage_municipios s
    WHERE f.codigo = trim(s.codigo_int)::int
...
AS $procedure$
BEGIN
    INSERT INTO cnpj.natureza (codigo, nome, hash)
    SELECT
        s.codigo::int,
        s.nome,
        md5(coalesce(s.nome,''))
...
AS $procedure$
BEGIN
    INSERT INTO cnpj.qualificacao (codigo, nome, hash)
    SELECT
        s.codigo::int,
        s.nome,
        md5(coalesce(s.nome,''))
